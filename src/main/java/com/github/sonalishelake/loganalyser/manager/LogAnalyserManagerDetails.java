package com.github.sonalishelake.loganalyser.manager;
...
import com.github.sonalishelake.loganalyser.repository.AlertRepository;
...
@Component
public class LogAnalyserManagerDetails {
    private static final Logger LOGGER = LoggerFactory.getLogger(LogAnalyserManagerDetails.class);
    ...
    public void parseAndPersistEvents(ContextLogFile context) {
        ...
        try (LineIterator li = FileUtils.lineIterator(new ClassPathResource("inputfile/" + context.getLogFilePath()).getFile())) {
            String line = null;
            while (li.hasNext()) {
                EventInfo event;
                try {
                    event = new ObjectMapper().readValue(li.nextLine(), EventInfo.class);
                    LOGGER.trace("{}", event);
                    ...
                    if (eventMap.containsKey(event.getId())) {
                        EventInfo e1 = eventMap.get(event.getId());
                        ...
                        if (executionTime > applicationDetails.getAlertThresholdMs()) {
                ...
                if (alerts.size() > applicationDetails.getTableRowsWriteoffCount()) {
    ...
    private long getEventExecutionTime(EventInfo event1, EventInfo event2) {
        EventInfo endEvent = Stream.of(event1, event2).filter(e -> StateInfo.FINISHED.equals(e.getState())).findFirst().orElse(null);
        EventInfo startEvent = Stream.of(event1, event2).filter(e -> StateInfo.STARTED.equals(e.getState())).findFirst().orElse(null);
